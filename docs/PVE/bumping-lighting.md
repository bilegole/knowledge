...
> 面向 K.D.S（宽带山）军团与 DC 联盟内的新老成员。本页讲清楚如何用“电鱼（智能炸弹刷怪）”与“炸鱼（轰炸机投弹刷怪）”在零安进行高效生产：原理、收益、配船、投入、安全与操作流程。遵循联盟情报与舰队规范，禁止任何违禁自动化/广播输入。

## 1. 概述与适用人群
- 电鱼：使用战列/战巡等搭载多枚大型智能炸弹，在异常中将聚拢的小型/中型 NPC 扫成片。优点是节拍稳定、容错高、单号也能做；缺点是需要合适场地与一定电容/抗性投入。
- 炸鱼：使用隐轰（轰炸机）以“投弹 + 鱼雷收尾”进行群体秒杀。更适合多号协同或小队分工（1 拉怪 + 3–7 炸机）。优点是爆发高、门槛低（单船便宜）；缺点是节拍被2分钟换弹限制，多号收益更明显。
- 适用：想在管内稳定刷赏金的新手与多号玩家；已能单刷 Hub/Haven 的人进一步提升效率；不想复杂操作、倾向“节拍化”生产者。

## 2. 基本术语与原理
- 智能炸弹（Smartbomb）：以自身为球形中心的范围伤害，伤害类型固定，周期性消耗大量电容；不需锁定目标，友伤开启需注意。对体型小、密集的 NPC 效果最好。
- 炸弹（Bomb）：隐轰专属武器，发射后沿直线飞行、在 30km 左右引爆造成范围伤害；同类炸弹7枚以上会互相引爆削弱；对聚堆小船/中船极有效。投弹后有约 120 秒换弹冷却。
- 赏金节拍（Tick）：系统每 20 分钟结算一次赏金。生产时建议以“每 Tick 输出”衡量效率，并记录 3 个 Tick 的平均值。
